...
struct FixedPanelsApp {
    ...
    graph_height: f32,      // Высота области для графика
    ...
    // ДАННЫЕ ДЛЯ ГРАФИКА
    graph_data: Vec<f32>,     // Данные для отображения на графике
}
...
impl FixedPanelsApp {
    fn new() -> Self {
        ...
        for i in 0..100 {
            let x = i as f32 / 10.0;
            graph_data.push((x.sin() + 1.0) * 500.0); // Синусоида от 0 до 1000
        }
        ...
        Self {
            ...
            graph_height: 300.0,     // Высота области для графика
    ...
    fn update_graph_data(&mut self) {
        // Добавляем новую точку на основе текущей позиции
        let new_value = self.slider_current + (rand::random::<f32>() - 0.5) * 100.0;
        self.graph_data.push(new_value);
        
        // Ограничиваем количество точек для производительности
        if self.graph_data.len() > 200 {
...
impl eframe::App for FixedPanelsApp {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            // Вертикальная компоновка: верхние панели, потом график
            ui.vertical(|ui| {
                // ВЕРХНЯЯ ЧАСТЬ С ПАНЕЛЯМИ (используем ScrollArea)
                ...
                let graph_response = ui.allocate_response(
                    egui::vec2(graph_width, self.graph_height),  // Фиксированная ширина
                    egui::Sense::hover()
                );
